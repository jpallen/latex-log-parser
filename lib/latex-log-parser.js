define(function() {
    ...
    (function() {
        ...
        this.currentLineIsBiberWarning = function () {
            return !!(this.currentLine.match(/^Package biblatex Warning: /));
        };
        ...
        this.parseBiberWarningLine = function() {
            ...
            var warningMatch = this.currentLine.match(/^Package biblatex Warning: (.*)$/);
            ...
            // Ok, now we are going to try and make the missing citation
            // message cleaner (it's a bit hardcoded but should be safe unless
            // biber changes it's output).
            // Otherwise, fallback to the raw message.
            var message = "";
            var raw_message = warning_lines.join(' ');
            if (warning_lines[1].indexOf('database') > -1) {
                message = "Citation '" + warning_lines[2] + "' undefined.";
            } else {
                message = raw_message;
            }
